class Picture {
    ...
    // Метод для создания массива, где все пикселы имеют один цвет
    static empty(width, height, color) {
        // Создает пустой массив и заполняет его цветом
...
// Рисуем каждый пиксель размером 10х10
const scale = 10;
...
function historeUpdateState(state, action) {
    if (action.undo == true) {
...
function startPixelEditor({
    ...
    let app = new PixelEditor(state, {
        ...
        controls,
        dispatch(action) {
            state = historeUpdateState(state, action);
            app.syncState(state);
        }
    });
    return app.dom;
